    ...
    for c_n_idx in range(1, len(all_nodes)):
        for c_i_idx in range(c_n_idx):
            ...
            if mahalanobis_dist < MAHALANOBIS_DISTANCE_TEST:
                img_name = '{:06d}.png'.format(min(c_n_idx * 19, 3449))
                orig_path0 = os.path.join(DATA_PATH, 'image_0', img_name)
                orig_path1 = os.path.join(DATA_PATH, 'image_1', img_name)
                shutil.copyfile(orig_path0, f'new_0/{img_name}')
                shutil.copyfile(orig_path1, f'new_1/{img_name}')

                img_name = '{:06d}.png'.format(min(c_i_idx * 19, 3449))
                orig_path0 = os.path.join(DATA_PATH, 'image_0', img_name)
                orig_path1 = os.path.join(DATA_PATH, 'image_1', img_name)
                shutil.copyfile(orig_path0, f'new_0/{img_name}')
                shutil.copyfile(orig_path1, f'new_1/{img_name}')
